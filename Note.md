...
- 闭包是一种特殊的内嵌函数，在一个内部函数里对外层非全局作用域的变量进行引用
...
- 类对象：创建一个类，其实也是一个对象在内存开辟了一块空间，称为类对象，类对象只有一个
...
- 若__new__没有正确返回当前类cls的实例，则__init__是不会被调用的，即使是父类的实例也不行，将没有__init__被调用
